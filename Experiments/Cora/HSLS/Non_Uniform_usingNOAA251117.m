...
% 关键：自动识别"行 = N 个节点"，候选与结果均做越界清洗；基于 learned_weights 打印 Top-98
...
%% 1) 读取输入信号 X
data_path = 'E:\experimentAndres251112\inverse_result.mat';
...
% ---------- 这里开始是"自动识别 N"，不再强制等于 48 ----------
[m, n] = size(X_raw);

% 约定：节点数 N 取"行数和列数中较大的一维"
% 如果列数更大，就转置，让"行 = 节点数"，列 = 样本数
if m >= n
    X_v = X_raw;      % 行已经是节点
else
    X_v = X_raw.';    % 转置，让较大的一维放到行
end

[N, P] = size(X_v);   % N 个节点，P 个样本/时间点

if N < 2
    error('当前数据只有 N=%d 个节点，至少需要 2 个节点。', N);
end
