function z = smooth_abs_sum_directed(X, listUnique)
%% 有向超图的Sum-Absolute变分 (对应论文中的Ω2)
% 根据公式(11)计算：
% z_ei = sum{||[X_k,: - X_o,:]_+||_1} + sum{||X_o,: - X_j,:||_1} + sum{||X_k,: - X_l,:||_1}
% 其中：
%   - 第一项：tail → head的距离求和（带投影）
%   - 第二项：tail内部节点之间的距离求和
%   - 第三项：head内部节点之间的距离求和
%
% 输入:
...
for i = 1:D
    ...
    summation = 0;  % 累积所有距离的总和
    
    %% 第一部分：Tail → Head 的距离求和（带投影 [·]_+）
    % 对应公式(11)中的第一项：sum_{v_o∈O_e, v_k∈I_e} ||[X_k,: - X_o,:]_+||_1
    ...
    %% 第二部分：Tail内部节点之间的距离求和
    % 对应公式(11)中的第二项：sum_{v_o, v_j ∈ O_e} ||X_o,: - X_j,:||_1
    ...
    %% 第三部分：Head内部节点之间的距离求和
    % 对应公式(11)中的第三项：sum_{v_k, v_l ∈ I_e} ||X_k,: - X_l,:||_1
